...
Full button-driven UI, admin management, ban system, announcements, and replies with media.
"""
import os
import sys
import logging
...
# ---------- Handler for forwarded media (admin reply with media) ----------
async def handle_forwarded_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If the user sends a numeric ID, we forward the stored media
    ...
    # If it's a forwarded message or any media, store it
    # We also store text messages as media (so admin can forward a text)
    if update.message.forward_origin or update.message.photo or update.message.video or update.message.document or update.message.text:
        ...
        # If it's a regular text that is not a user ID, we might ignore
        await update.message.reply_text("❌ Please forward a message or send a numeric user ID.")
...
application.add_handler(MessageHandler(filters.ALL, handle_forwarded_reply))  # catch all messages for reply media
